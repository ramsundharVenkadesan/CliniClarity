...
async def hallucination(state: GraphState) -> Dict[str, Any]: # Function to perform the checks
    logging.info("Running DeepEval Faithfulness Check!")
    ...
    logging.info("Evaluating Groundedness using DeepEval...")
    ...
    try: # Attempt to perform evaluation
        ...
        else: await asyncio.to_thread(metric.measure, test_case)
        ...
        logging.error(f"DeepEval Execution Failed: {e}")
        score = 0.0 # Score set to 0
        return e

    logging.info(f"DeepEval Score: {score}") # Final score to console

    if score > 0.9: # Score is above 90%
        logging.info("Decision: ✅ ACCEPTED (Grounded in context)")
    else: # The score is below 90%
        logging.warning("Decision: ❌ REJECTED (Hallucination Detected)")# Hallucination detected # Assign the hallucinated summary
