...
def reflection_node(state:MessageGraph) -> Dict[str, Any]: # Node function to reflect on the generated summary
    ...
    try: # Code block to contain erroneous code
        ...
        if reflection_result.is_approved: # The node approves the result
            ...
            print("   ❌ VERDICT: FAILED. Issuing Corrective Memo.") # The summary failed the checks
            print(f"   📝 MEMO: {reflection_result.corrective_memo}") # The corrective memo is returned by the LLM
        ...
        with open(ACTIVITY_LOG_FILE, mode='a', encoding='utf-8') as logging_file: # Open logging file in append mode
            print(f"[{datetime.datetime.now()}] PARSING ERROR: Auditor failed to output valid JSON. Forcing retry. ({str(e)})", file=logging_file) # Write error details to the logging file
...
def should_audit(state:MessageGraph) -> str: # Router function to determine routes between nodes
    ...
    if len(message) > 8: # Length of messages list is greater than 8
        with open(ACTIVITY_LOG_FILE, mode='a', encoding='utf-8') as logging_file: # Open the logging file in append mode
            print("\n🛑 ROUTER: Maximum retries reached (3 loops). Forcing exit.", file=logging_file) # Write the error details to logging file
        print("\n🛑 ROUTER: Maximum retries reached (3 loops). Forcing exit.") # Exiting the reflection-loop
        return END # Return the terminator node
    elif "PASSED" in last_message.content: # Check if the summary passes the checks
        print("\n🏁 ROUTER: Audit complete. Finalizing report for UI.") # Audit has been completed
        return END # Return the terminator node
